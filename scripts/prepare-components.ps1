...
function Download-File([string]$Url, [string]$OutFile) {
    ...
    & curl.exe -L --fail --retry 3 --connect-timeout 30 -A "Mozilla/5.0 STAVCMS-Build" -o $OutFile $Url
    if ($LASTEXITCODE -ne 0) { throw "Download failed: $Url" }
    if (-not (Test-Path $OutFile)) { throw "Downloaded file missing: $OutFile" }
    $size = (Get-Item $OutFile).Length
    if ($size -lt 1MB) { throw "Downloaded file is unexpectedly small ($size bytes): $Url" }
}
...
if (-not (Test-Path $apacheSource)) { throw 'Apache24 directory not found in Apache archive.' }
...
if (-not $mariaSource) { throw 'MariaDB directory not found in archive.' }
